...
public class WeaponType : ScriptableObject
{
	public string Name;
	public int ItemID;
	public bool IsBlip = false;
	public bool staticSpeed = true;
	...
	[HideInInspector]
	public int indexemitt = 0;

	public WeaponType(WeaponType Oldtype)
	{
		Name = Oldtype.Name;
	...
	public void shoot()
	{
		...
		if (!AlternateEmitPoints)
		{
			...
			foreach (Vector3 Emitter in EmitterPos) 
			{
				...
				if (IsBlip)
				{
					Entity targetship = null;
					Entity[] Entitys = FindObjectsOfType<Entity>() as Entity[];
					float LastClose = 100.0f;
					foreach (Entity entity in Entitys) 
					{
						if (Mathf.Abs(entity.transform.position.x - ThisObject.transform.position.x) < LastClose && entity.gameObject.layer != ThisObject.gameObject.layer && entity != null)
						{
							targetship = entity as Entity;
							LastClose = Mathf.Abs(entity.transform.position.x - ThisObject.transform.position.x);
						
						}
					}
					Projectile_Spawned.GetComponent<Rigidbody>().AddRelativeForce((ThisObject.transform.position - EmitterPos[this.indexemitt] - targetship.gameObject.transform.position).normalized * WeaponSpeed);
				}
				else 
				{
				...
				Projectile_Spawned.GetComponent<ProjectileController>().staticSpeed = staticSpeed;
				Projectile_Spawned.GetComponent<ProjectileController>().WeaponSpeed = WeaponSpeed;
		...
		else ///Alternate Emitter
		{
			...
			if (ThisObject.GetComponent<Entity>().FaceDown)
			{
			...
			if (IsBlip)
			{
				Entity targetship = ThisObject.GetComponent<Entity>();
				Entity[] Entitys = FindObjectsOfType<Entity>() as Entity[];
				float LastClose = 100.0f;
				foreach (Entity entity in Entitys) 
				{
