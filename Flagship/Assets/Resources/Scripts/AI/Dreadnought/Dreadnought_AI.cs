...
[Serializable]
public class Dreadnought_AI : MonoBehaviour 
	{	
	public GameObject ExplosionObj;

	[HideInInspector]
	public DreadnoughtController thisShip;
	...
	void Start () 
	{
		thisShip = this.GetComponentInParent<DreadnoughtController> ();
	...
	public void Shoot(int Index)
	{
		...
		if (!WeaponList[Index].AlternateEmitPoints)
		{
			...
			foreach (Vector3 Emitter in WeaponList[Index].EmitterPos) 
			{
				...
				Projectile_Spawned.GetComponent<ProjectileController>().WeaponSpeed = WeaponList[Index].WeaponSpeed;
				Projectile_Spawned.GetComponent<ProjectileController>().staticSpeed = WeaponList[Index].staticSpeed;
